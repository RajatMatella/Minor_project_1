...
vector<int> make_path()
{
 ...
 int calc_fitness(vector<int> path,int map[CITIES][CITIES])
{
    int f = 0;
...
void solve_TSP(int map[CITIES][CITIES])
{
    ...
    cout<<"most optimal path: "<<*min_element(minimum_set.begin(), minimum_set.end())<<endl;
}
